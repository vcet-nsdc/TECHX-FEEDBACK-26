...
async function seedReviews() {
  try {
    ...
    const labDocs = await labsCol.find({ labKey: { $in: ['1', '2', '3', '4'] } }).toArray();
    
    // Mapping of labKey to canonical lab id: '1' -> 'a', '2' -> 'c', '3' -> 'd', '4' -> 'e'
    const CANONICAL_MAP = { '1': 'a', '2': 'c', '3': 'd', '4': 'e' };

    const allProducts = [];
    const productsByLab = { '1': [], '2': [], '3': [], '4': [] };
    ...
    for (const doc of labDocs) {
      ...
      for (const cp of checkpoints) {
        const item = {
          ...
          labName: doc.lab?.name || `Lab ${labKey === '1' ? '502' : labKey === '2' ? '508' : labKey === '3' ? '509' : '510'}`,
        };
